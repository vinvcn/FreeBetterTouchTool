...
|---|---|---|
| Project definition | Defines scope, constraints, execution model, acceptance target | `ai_context/`, `AGENTS.md`, `README.md` |
| Agent state harness | Tracks current decision, state transition rules, next action, definition of done | `agent_state_harness/` |
| MVP user flow harness | Captures functional user flows, P0/P1 priorities, and failure modes | `mvp_user_flow_harness/` |
| Engineering flow | Breaks execution, acceptance, and release into controlled nodes | `project_control/`, `codex_tasks/`, `gate_checklists/` |
| Autopilot control | Minimizes owner intervention while keeping gates and human acceptance | `codex_automation/`, `owner_control/`, `acceptance/` |
...
- `ai_context/engineering_execution_model.md`
- `mvp_user_flow_harness/p0_p1_user_flows.md`
